...
export default function Dashboard() {
  ...
  // 📅 Load history (Consistency Tracker)
  const loadHistory = useCallback(async () => {
    try {
      const res = await API.get("/tasks/history/all");
      setHistory(res.data || []);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #3698e3 0%, #ffffff 50%, #f3f7ff 100%)" }}>
      <nav className="navbar bg-dark navbar-dark px-4 mb-4">
      ...
      <div className="container pb-5" style={{ maxWidth: "850px" }}>
        ...
        <ul className="nav nav-pills justify-content-center gap-2 mb-4">
          {["today", "daily", "weekly"].map((type) => (
            <li className="nav-item" key={type}>
              <button className={`nav-link px-4 fw-semibold ${taskType === type ? "active shadow" : "bg-white text-dark border"}`}
                onClick={() => setTaskType(type)}>{type.toUpperCase()}</button>
            </li>
          ))}
        </ul>

        {/* Add Task Card */}
        <div className="card border-0 mb-4 shadow" style={{ borderRadius: "16px" }}>
          <div className="card-body p-4">
            <h5 className="fw-bold mb-3">New {taskType.toUpperCase()} Task</h5>
            <div className="row g-2">
              <div className="col-md-5"><input className="form-control" placeholder="Title" value={title} onChange={(e) => setTitle(e.target.value)} /></div>
              <div className="col-md-5"><input className="form-control" placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)} /></div>
              <div className="col-md-2"><button className="btn btn-dark w-100" onClick={addTask}>Add</button></div>
            </div>
          </div>
        </div>
        ...
        <div className="input-group mb-4 shadow-sm">
          ...
          <input className="form-control" placeholder={`Search ${taskType} tasks`} value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        ...
        {filteredTasks.length === 0 ? (
          ...
          filteredTasks.map((task) => (
            <div key={task._id} className="card mb-3 border-0 shadow-sm" style={{ borderLeft: "5px solid #111827", borderRadius: "12px" }}>
              <div className="card-body d-flex justify-content-between align-items-center">
                <div>
                ...
                <div className="d-flex gap-2">
                  <button className="btn btn-outline-success btn-sm rounded-pill" onClick={() => markDone(task._id)}>Done</button>
                  <button className="btn btn-outline-danger btn-sm" onClick={() => deleteTask(task._id)}><i className="bi bi-trash"></i></button>
                </div>
              </div>
            </div>
          ))
        ...
        {/* Consistency Tracker */}
        <div className="card border-0 mt-5 shadow" style={{ borderRadius: "16px" }}>
          <div className="card-body p-4 text-center">
            <h5 className="fw-bold mb-4">Consistency Tracker</h5>
            <div className="d-flex justify-content-center">
              <Calendar tileClassName={tileClassName} />
            </div>
          </div>
